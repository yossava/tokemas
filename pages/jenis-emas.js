...
function JenisEmas() {
    ...
    const getCust = async (c = cookies.jwt, p = page) => {
        ...
        try {
            var res = await axios.get(`${process.env.API_URL}/api/gold-types/?pagination[pageSize]=${size}&pagination[page]=${p}${searchVal}`,
                {
                    headers: {
    ...
    const deleteResource = async (id) => {
        try {
            var res = await axios.delete(`${process.env.API_URL}/api/gold-types/${id}`,
                {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            var res = await axios.post(process.env.API_URL + '/api/gold-types/', { data: userData },
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            var res = await axios.put(process.env.API_URL + '/api/gold-types/' + editId, { data: editData },
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
    ...
    async function editResource(id) {
        setEditId(id);
        try {
            var res = await axios.get(process.env.API_URL + '/api/gold-types/' + id,
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
            ...
            {owner && <div>
                {editId == 0 && <div>
                    <div className='w-full p-3 rounded'>
                        <p className='font-bold text-lg'>Buat Jenis Baru</p>
                    </div>
                ...
                <div className='rounded p-3'>
                    <div className='w-full rounded my-3'>
                        <p className='font-bold text-lg'>Daftar Nama</p>
                    </div>
