...
function Customer() {
    ...
    const getCust = async (c = cookies.jwt, p = page) => {
        let size = 10;
        let searchVal = `&filters[$or][0][no_hp][$containsi]=${search}&filters[$or][1][nama][$containsi]=${search}`;
        if (search != "") {
            p = 1;
            size = 100;
        }
        try {
            var res = await axios.get(`${process.env.API_URL}/api/customers?pagination[pageSize]=${size}&pagination[page]=${p}${searchVal}`,
                {
                    headers: {
    ...
    const deleteResource = async (id) => {
        try {
            var res = await axios.delete(`${process.env.API_URL}/api/customers/${id}`,
                {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.success("Customer berhasi ditambahkan", { position: toast.POSITION.TOP_CENTER });
            getCust(cookies.jwt);
        } catch (err) {
            console.log(err);
        }
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            var res = await axios.put(process.env.API_URL + '/api/customers/' + editId, { data: editData },
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
    ...
    async function editResource(id) {
        setEditId(id);
        try {
            var res = await axios.get(process.env.API_URL + '/api/customers/' + id,
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
        ...
        return custArr.map((el) => {
            return (
                <div key={el.id} className='grid grid-cols-6 gap-3 text-left text-xs border-b py-3'>
                    <div>{el.id}</div>
                    <div>{el.attributes.nama}</div>
                    <div>{el.attributes.no_hp}</div>
                    <div>{el.attributes.alamat}</div>
                    <div>{el.attributes.email}</div>
    ...
    return (
        <div>
            ...
            {owner && <div>
                {editId == 0 && <div>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className='rounded p-3 grid grid-cols-2 gap-3'>
                            ...
                            <input onChange={(e) => handleChange(e)} required placeholder='No HP' name="no_hp" className='p-2 border rounded' />
                            <input onChange={(e) => handleChange(e)} placeholder='Alamat' name='alamat' className='p-2 border rounded' />
                            <input onChange={(e) => handleChange(e)} placeholder='Email' name='email' className='p-2 border rounded' />
                            <div></div>
                ...
                {editId != 0 && <div>
                    <div className='w-full p-3 rounded'>
                        <div className='font-bold text-lg'>Edit Customer <button onClick={() => setEditId(0)} className='text-xs border p-1'>Buat Baru</button></div>
                    </div>
                    <form onSubmit={handleUpdate}>
                        <div className='rounded p-3 grid grid-cols-2 gap-3'>
                            ...
                            <input onChange={(e) => handleUpdateChange(e)} required placeholder='No HP' value={editData?.no_hp || ""} name="no_hp" className='p-2 border rounded' />
                            <input onChange={(e) => handleUpdateChange(e)} placeholder='Alamat' name='alamat' value={editData?.alamat || ""} className='p-2 border rounded' />
                            <input onChange={(e) => handleUpdateChange(e)} placeholder='Email' name='email' value={editData?.email || ""} className='p-2 border rounded' />
                            <div></div>
                ...
                <div className='rounded p-3'>
                    <div className='w-full rounded my-3'>
                        <p className='font-bold text-lg'>Daftar Customer</p>
                    </div>
                    <div className='mb-3 flex'>
                        <input onChange={(e) => setSearch(e.target.value)} placeholder='Cari Customer' className='w-4/12 p-2 mr-2 border rounded' />
