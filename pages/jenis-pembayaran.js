...
function JenisPembayaran() {
    ...
    const getCust = async (c = cookies.jwt, p = page) => {
        ...
        try {
            var res = await axios.get(`${process.env.API_URL}/api/payment-types/?pagination[pageSize]=${size}&pagination[page]=${p}${searchVal}`,
                {
                    headers: {
    ...
    const deleteResource = async (id) => {
        try {
            var res = await axios.delete(`${process.env.API_URL}/api/payment-types/${id}`,
                {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            var res = await axios.post(process.env.API_URL + '/api/payment-types/', { data: userData },
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
                });
            toast.success("Nama berhasi ditambahkan", { position: toast.POSITION.TOP_CENTER });
            getCust(cookies.jwt);
        } catch (err) {
            console.log(err);
        }
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            var res = await axios.put(process.env.API_URL + '/api/payment-types/' + editId, { data: editData },
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
    ...
    async function editResource(id) {
        setEditId(id);
        try {
            var res = await axios.get(process.env.API_URL + '/api/payment-types/' + id,
                {
                    headers: {
                        Authorization: 'Bearer ' + cookies.jwt
                    }
            ...
            {owner && <div>
                {editId == 0 && <div>
                    <div className='w-full p-3 rounded'>
                        <p className='font-bold text-lg'>Buat Pembayaran Baru</p>
                    </div>
                ...
                {editId != 0 && <div>
                    <div className='w-full p-3 rounded'>
                        <div className='font-bold text-lg'>Edit Nama <button onClick={() => setEditId(0)} className='text-xs border p-1'>Buat Baru</button></div>
                    </div>
                ...
                <div className='rounded p-3'>
                    <div className='w-full rounded my-3'>
                        <p className='font-bold text-lg'>Daftar Pembayaran</p>
                    </div>
                    <div className='mb-3 flex'>
                        <input onChange={(e) => setSearch(e.target.value)} placeholder='Cari Nama' className='w-4/12 p-2 mr-2 border rounded' />
